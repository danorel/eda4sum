...
parser.add_argument(
    "--utility_weights", nargs="+", type=float, default=[0.333, 0.333, 0.334]
)
...
class Agent:
    def __init__(self, env_name):
        ...
        if args.dataset == "sdss":
            ...
            ds_name = "tracks_7c"
            columns = [
                f"{ds_name}.popularity",
                f"{ds_name}.acousticness",
                f"{ds_name}.danceability",
                f"{ds_name}.duration_ms",
                f"{ds_name}.energy",
                f"{ds_name}.instrumentalness",
                f"{ds_name}.liveness",
            ]  # , f"{ds_name}.loudness", f"{ds_name}.speechiness", f"{ds_name}.tempo", f"{ds_name}.valence", ]
            self.pipeline: PipelineWithPrecalculatedSets = (
        ...
        if args.resume:
            ...
            self.global_set_actor = SetActor(
                ...
                args.actor_lr,
                self.agent_name,
                model_path=model_path + "set_actor",
            )
            self.global_operation_actor = OperationActor(
                ...
                args.actor_lr,
                self.agent_name,
                model_path=model_path + "operation_actor",
            )
    ...
    def evaluate(self):
        ...
        for episode_counter in range(eval_length):
            ...
            for step_counter in tqdm(range(self.episode_steps)):
                ...
                reward = (
                    args.counter_curiosity_ratio * float(intrinsic_reward)
        ...
        if mean_reward > self.global_best_evaluation_so_far["mean_reward"]:
            ...
            with open(
                f"saved_models/{args.name}/best_reward/set_op_counters.json", "w"
            ) as f:
        ...
        if mean_utility > self.global_best_evaluation_so_far["mean_utility"]:
            ...
            with open(
                f"saved_models/{args.name}/best_utility/set_op_counters.json", "w"
            ) as f:
                json.dump(self.global_set_op_counters, f, indent=1)
        if mean_score > self.global_best_evaluation_so_far["mean_score"]:
            ...
            with open(
                f"saved_models/{args.name}/best_score/set_op_counters.json", "w"
            ) as f:
                json.dump(self.global_set_op_counters, f, indent=1)
            ...
            try:
                for step_counter in tqdm(range(self.episode_steps)):
                    ...
                    if args.curiosity_ratio > 0:
                        ...
                        reward = (
                            args.curiosity_ratio * float(intrinsic_reward)
                    ...
                    if len(set_state_batch) >= args.update_interval or done:
                        ...
                        advantages = td_targets - self.critic.model.predict(set_states)
                        ...
                        with self.lock:
                            try:
                                ...
                                operation_actor_loss = (
                                    self.global_operation_actor.train(
                                ...
                                if (
                                    ...
                                    with open(
                                        f"saved_models/{args.name}/{ep}/set_op_counters.json",
                                        "w",
                                    ) as f:
                                        json.dump(
                                ...
                                if (
                                    ...
                                    and CUR_EPISODE != 0
                                    and CUR_EPISODE % args.eval_interval == 0
                                ):
                                    self.evaluate()

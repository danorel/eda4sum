...
class PipelineEnvironment(gym.Env):
    def __init__(
        ...
        if self.target_set_name != None and target_items == None:
            with open(
                f"./rl/targets/{self.database_name}/{self.target_set_name}.json"
            ) as f:
            ...
            self.state_encoder = StateEncoder(
                ...
                target_items=TargetSetGenerator.get_diverse_target_set(
                    self.pipeline.database_name, number_of_samples=100
                ),
            )
    ...
    def get_target_set_and_examples(self):
        ...
        self.target_set_index = available_target_sets.index(self.target_set_name)
        with open(
            f"./rl/targets/{self.database_name}/{self.target_set_name}.json"
        ) as f:

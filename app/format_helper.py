...
class FormatHelper:
    @classmethod
    def get_interval_predicate_item(
        cls, attribute: str, filters: List[FilterDefinition], pipeline: Pipeline
    ):
        low_value = None
        high_value = None
    ...
    def get_predicate_item(cls, filter_definition: FilterDefinition):
        ...
        if filter_definition.leftOperand.type == OperandType.Column:
            ...
            value = (
                float(filter_definition.rightOperand.value)
                if filter_definition.rightOperand.type == OperandType.Number
                else filter_definition.rightOperand.value
            )
        else:
            attribute = filter_definition.rightOperand.value
            value = (
                float(filter_definition.leftOperand.value)
                if filter_definition.leftOperand.type == OperandType.Number
                else filter_definition.leftOperand.value
            )
            ...
            if len(dataset.tables) > 1:
                ...
                for table in tables:
                    relations = pipeline.foreign_keys[
                        (
                            (pipeline.foreign_keys["table1"].isin(joined_tables))
                            & (pipeline.foreign_keys["table2"] == table)
                        )
                        | (
                            (pipeline.foreign_keys["table2"].isin(joined_tables))
                            & (pipeline.foreign_keys["table1"] == table)
                        )
                    ]
                    ...
                    if len(relations) == 1:
                        ...
                        if tables.index(table) == len(tables) - 1:
                            raise Exception("Unable to join " + table)
                        tables.append(table)
                    else:
                        raise Exception("Multiple relations possible to join " + table)
                ...
                if len(filter_list) == 1:
                    ...
                    if attribute in pipeline.interval_indexes:
                        dataset.predicate.append(
                            cls.get_interval_predicate_item(
                                attribute=attribute,
                                filters=filter_list,
                                pipeline=pipeline,
                            )
    ...
    def get_sql_query(cls, pipeline: Pipeline, dataset: Dataset):
        tables = (
            [pipeline.initial_collection_names[0]]
            + list(map(lambda x: x.target_collection_name, pipeline.initial_joins))
            + list(map(lambda x: x.target_collection_name, dataset.joins))
        )

...
app.mount("/dora-summaries", StaticFiles(directory="client", html=True), name="client")
...
async def get_target_items_and_prediction(
    target_set: str = None, curiosity_weight: float = None, dataset_ids: List[int] = []
):
...
async def load_model(galaxy_request: OperatorRequest):
    ...
    prediction_results = model_manager.get_prediction(
        datasets,
        galaxy_request.weights_mode,
        galaxy_request.target_items,
        galaxy_request.found_items_with_ratio,
        previous_set_states=galaxy_request.previous_set_states,
        previous_operation_states=galaxy_request.previous_operation_states,
    )

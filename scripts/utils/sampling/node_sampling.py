...
def node_sampling_pipeline(
    ...
    pipeline_body: t.List[PipelineBodyItem],
    sample,
...
def node_sampling_csv(
    ...
    for attempt in range(attempts):
        ...
        item_set = set([int(id) for id in members.iloc[input_set_id, 2][1:-1].split(", ")])
        if satisfies_conditions(item_set):
            logger.info(f"Node[id={input_set_id}] satisfies the conditions (size = {len(item_set)})")
            break
        else:
            logger.info(f"Node[id={input_set_id}] violates the conditions (size = {len(item_set)}). Attempt #{attempt}...")

    if not len(item_set):
        logger.warn(f"Not found node satisfying conditions ({attempts} attemts done)")
        return None
